package com.android.baoliyota.network.convert;
...
public class FileConvert implements Converter<File> {
    ...
    public File convertSuccess(Response value) throws Exception {
        ...
        if (TextUtils.isEmpty(destFileName)) destFileName = BaoliYotaHttpUtils.getNetFileName(value, value.request().url().toString());
        ...
        try {
            ...
            while ((len = is.read(buf)) != -1) {
                ...
                if (callback != null) {
                    ...
                    //每200毫秒刷新一次数据
                    if (curTime - lastRefreshUiTime >= BaoliYotaHttpManager.REFRESH_TIME || finalSum == total) {
                        //计算下载速度
